  ...
  bool outsideKernel(std::shared_ptr<GridPoint> grid_point,
    ...
    if (r2 > kernel_rad2) {
      for (int p = 0; p < this->part_count; p++) {
        ...
        if (p_r2 <= kernel_rad2) {
          message("Cell: loc = %f %f %f, width = %f %f %f", this->loc[0],
                  this->loc[1], this->loc[2], this->width[0], this->width[1],
                  this->width[2]);
          message("Grid point: loc = %f %f %f", grid_point->loc[0],
                  grid_point->loc[1], grid_point->loc[2]);
          message("Particle: pos = %f %f %f", part->pos[0], part->pos[1],
                  part->pos[2]);
          error("Particle inside kernel radius but cell outside (dx=%f, dy=%f, "
                "dz=%f, r2 = %f, kernel_rad2 = %f)",
                dx, dy, dz, r2, kernel_rad2);
        }
      }
    }
#endif
