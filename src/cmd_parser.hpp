...
public:
   ...
   */
  static CommandLineArgs parse(int argc, char* argv[], int rank = 0, int size = 1) {
  ...
  static void printUsage(const char* program_name) {
    std::cerr << "\nUsage: " << program_name << " <parameter_file> <nthreads> [snapshot_number]\n\n";
    std::cerr << "Arguments:\n";
    std::cerr << "  parameter_file   Path to YAML parameter configuration file\n";
    std::cerr << "  nthreads        Number of OpenMP threads (1-" << getMaxThreads() << ")\n";
    std::cerr << "  snapshot_number Optional snapshot number (>=0, replaces placeholder)\n\n";
    std::cerr << "Options:\n";
   ...
   */
  static void printError(const std::string& error_msg, const char* program_name) {
    std::cerr << "\nError: " << error_msg << "\n";
...
private:
  ...
  static void validateArgumentCount(int argc, const char* /* program_name */) {
    if (argc < 3 || argc > 4) {
      ...
      oss << "Invalid number of arguments (" << (argc-1) << "). Expected 2-3 arguments.";
  ...
  static int parseThreadCount(const char* arg) {
    ...
    try {
      ...
      if (pos != std::strlen(arg)) {
        throw std::runtime_error("Thread count contains non-numeric characters");
      }
    ...
    // Validate range
    if (nthreads <= 0) {
      throw std::runtime_error("Thread count must be positive (got " + std::to_string(nthreads) + ")");
    }
    
    const int max_threads = getMaxThreads();
    if (nthreads > max_threads) {
      std::ostringstream oss;
      oss << "Thread count " << nthreads << " exceeds recommended maximum " << max_threads;
      throw std::runtime_error(oss.str());
    }
  ...
  static int parseSnapNumber(const char* arg) {
    ...
    try {
      ...
      // Check if entire string was consumed
      if (pos != std::strlen(arg)) {
        throw std::runtime_error("Snapshot number contains non-numeric characters");
      }
    } catch (const std::invalid_argument&) {
    ...
    // Validate range
    if (nsnap < 0) {
      throw std::runtime_error("Snapshot number must be non-negative (got " + std::to_string(nsnap) + ")");
    }
    
    // Reasonable upper limit check
    if (nsnap > 9999) {
      std::cerr << "Warning: Snapshot number " << nsnap << " is unusually high\n";
    }
  ...
  static void validateInputs(const CommandLineArgs& args, int rank, int size) {
    ...
    if (rank == 0 && hardware_threads > 0 && total_threads > hardware_threads * 2) {
  ...
  static int getMaxThreads() {
    const int hardware_threads = std::thread::hardware_concurrency();
    return hardware_threads > 0 ? hardware_threads * 2 : 64;  // Fallback to reasonable limit
  }
};
