...
GridGenerator class from the grid_smoother module. It supports various kernel
diameters for a fixed set of grid points.

Example usage:

    mpirun -np 4 generate_regions.py --input /IN/snapshot.hdf5 
                                     --output /OUT/grid.hdf5
...
Command Line Arguments:
    ...
    --kernel_diameters (List[float]):
        The diameter of the spherical region kernel in simulation units. 
        Required.
    --delete_distributed (int):
        Should the distributed files be deleted after combination?
        Default is 0.
    --nthreads (int):
        The number of threads to use within a rank. Default is 1.

Usage:
    python generate_regions.py --input /path/to/snapshot.hdf5 
                               --output /path/to/grid.hdf5
...
from grid_smoother import GridGenerator
...
def main():
    ...
    print("CHRIS: kernel diameters:", args.kernel_diameters)
